import { SOCIAL_DETAIL } from '@constants';
class PaymentGateway {
  ...
  allCard(id) {
    return new Promise((resolve, reject) => {
      this.callApi('https://api.stripe.com/v1/customers/' + id + '/sources?object=card', 'GET')
        .then(response => {
          if (response.error) {
            reject(response);
          } else {
  ...
  setPrimaryCard(cust_id, source) {
    ...
    return new Promise((resolve, reject) => {
      this.callApi('https://api.stripe.com/v1/customers/' + cust_id + '?default_source=' + source, 'POST')
        .then(response => {
          if (response.error) {
            reject(response);
          } else {
  ...
  deleteCard(cust_id, card_token) {
    return new Promise((resolve, reject) => {
      this.callApi('https://api.stripe.com/v1/customers/' + cust_id + '/sources/' + card_token, 'DELETE')
        .then(response => {
          if (response.error) {
            reject(response);
          } else {
  ...
  newCardCustomerAdd(cust_id, object) {
    ...
    return new Promise((resolve, reject) => {
      this.callApi('https://api.stripe.com/v1/customers/' + cust_id + '/sources', 'POST', formBody)
        .then(response => {
          if (response.error) {
            reject(response);
          } else {
